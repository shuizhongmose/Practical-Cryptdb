mysql -uroot -pletmein -h127.0.0.1 -e "drop database if exists sbtest"
rm -rf ./shadow/
mkdir ./shadow
...
run_with_nohup() {
    #### 后台运行
    nohup ./obj/main/basic_test > basic_test.log 2>&1 &
}
...
run_with_valgrind() {
    ...
    valgrind --tool=memcheck \
            ...
            ./obj/main/basic_test > basic_test.log 2>&1
}
...
## 使用valgrind+massif分析
run_with_heaptrack (){
  nohup heaptrack ./obj/main/basic_test > basic_test.log 2>&1 &
...
case $choice in
  ...
  7)
    ...
    ;;
  *)
    echo "Chose default run_without_nohup"
    run_without_nohup
    ;;
esac
